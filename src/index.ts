...
function loadUsers(): Account[] {
  try {
    ...
    const users = jsonData.map(
      (user: any) =>
        new Account(user.holder, user.balance, user.history, user.accountNumber)
    );
    return users;
  } catch (error) {
...
const users: Account[] = loadUsers(); // Array to store user accounts
...
async function performTransactions(user: Account) {
  while (true) {
    const answer = await inquirer.prompt([
      {
        type: "list",
...
async function performDeposit(user: Account) {
  const answer = await inquirer.prompt([
    {
      type: "number",
      name: "amount",
...
async function performWithdrawal(user: Account) {
  const answer = await inquirer.prompt([
    {
      type: "number",
      name: "amount",
...
async function performTransfer(user: Account) {
  const answer = await inquirer.prompt([
    {
      type: "number",
      name: "amount",
  ...
  if (recipient) {
    ...
    console.log(
      `$${amount} transferred to Account ${recipientAccountNumber} successfully.`
    );
    console.log(user.toString());
